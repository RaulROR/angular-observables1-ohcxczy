...
export class AppComponent {
  ...
  testOperadorFrom1() {
    var source = Observable.from(
      new Promise<string>( (resolve, reject) => {
          setTimeout(() => { resolve("Valor resuelto por la Promise."); }, 5000);
      })
    );
    source.subscribe(x => console.log(x));
  }
  ...
  testOperadorDo() {
    var source = Observable.of('a','b','c').do(x => console.log('Observable: '+ x)); //Emite secuencia ‘a’, ‘b’ y ‘c’, y muestra estos valores por log
    source.subscribe(x => console.log('Suscripción: ' +x));
  }
